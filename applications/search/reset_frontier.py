...
from datamodel.search.Haoc19Mutianx1_datamodel import Haoc19Mutianx1Link
...
@Deleter(Haoc19Mutianx1Link)
@Getter(Haoc19Mutianx1Link)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(Haoc19Mutianx1Link)
        print "Found ", len(ls), " links to delete."
        for l in ls:
            self.frame.delete(Haoc19Mutianx1Link, l)
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_Haoc19Mutianx1".format(CrawlerFrame.app_id),
